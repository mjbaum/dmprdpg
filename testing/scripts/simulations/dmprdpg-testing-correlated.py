...
parser.add_argument("-a", type=float, dest="a", default=0.0, const=True, nargs="?",\
	help="Float: Fraction of variance shared between matrix entries. Default: a=0.0")
...
def random_correlation_matrix(n: int, a: float, k: int = 10, rng=None, jitter=1e-8) -> np.ndarray:
    if rng is None:
        rng = np.random.default_rng()

    # Random factor matrix with normalised rows so each variable has unit variance
    F = rng.standard_normal((n, k))
    F /= np.linalg.norm(F, axis=1, keepdims=True)

    S = a * (F @ F.T) + (1 - a) * np.eye(n)
...
def sample_latent_gaussian(
    ...
    sigma = random_correlation_matrix(n * n, a=a, k=10)
    L = cholesky(sigma, lower=True)

    # Draw independent normals and colour them with the Cholesky factor
    z = L @ rng.standard_normal(n * n)

    return z.reshape(n, n)

def simulate_dmpsbm_correlated(n, B_dict, K=None, T=None, prior_G=None, prior_G_prime=None, seed=None, z_shared=False, undirected=False, a=0.0):
    ...
    for k in range(K):
        for t in range(T):
            Z = sample_latent_gaussian(n, a=a)
            if undirected:
                P = B_dict[k, t][np.ix_(z, z)]
                thresholds = norm.ppf(P)
                edges = np.triu(Z <= thresholds)
                edges = edges | edges.T
            else:
                P = B_dict[k, t][np.ix_(z, z_prime)]
                thresholds = norm.ppf(P)
                edges = Z <= thresholds
            rows, cols = np.where(edges)
            data = np.ones(len(rows))
            A_dict[k, t] = coo_matrix((data, (rows, cols)), shape=(n, n)).tocsr()
    ## Return output
    if undirected:
        return A_dict, z
    else:
...
for i in range(num_replicates):
    A_dict, z, z_prime = simulate_dmpsbm_correlated(n=n, B_dict=B_dict_equal, a=a)
...
np.savetxt(input_folder + f"/test_bootstrap_correlated_n={n}_eta={eta}_a={a}.csv", p_values, delimiter=",")